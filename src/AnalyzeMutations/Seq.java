...
public class Seq{
	...
	for(int i=0; i<this.seqBuffer.length(); i++){
		...
		if(count == chosenNthBase){
		    return this.getSeqName() + " " + (i+1) + " " + curChar + " " + makePutationArray(curChar, getMutChar(curChar, mutTypeIndex),numLines) + curChar + "\n";
	...
	for(int i=0; i<this.seqBuffer.length(); i++){
		...
		//count++;
		if(count == chosenNthBase){
		    System.out.println(this.getSeqName() + " " + (i+1) + " " + curChar + " " + makePutationArray(curChar, getMutChar(curChar, mutTypeIndex),numLines) + curChar );
		    //System.out.println(mutTypeIndex + "\t" + chosenNthBase + "\t" + targets[0]+"/"+targets[1] + "\t" + i);
		    return i;
		}
		count++;
