...
public class AnalyzeMutations{
    ...
    */
    public static void main(String[] args){
	if(args.length == 10){
	    //String[] pttList = this.getPttFiles(args[9]);
	    new AnalyzeMutations(args[1], args[2], args[3] , args[4], args[5], args[6], args[7], args[8], getPttFiles(args[9])).process(args[0]);
	}else
	...
	for(int i=0;i<this.seqHash.size();i++){
	    mutTable[i] = new StringBuffer("Line\tContig\tPosition\tRefBase\tConsensus\tMutBase\tbeforeTriplet\tafterTriplet\tbeforeAA\tafterAA\tgeneName\tgeneDirection\tMutType1\tMutType2\tConservative?\tSyn/non-syn\n");
